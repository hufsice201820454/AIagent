...
1. Loads allowed_companies.json whitelist
2. For OEMs: Individual stock charts (PNG)
3. For Suppliers (Battery/HVAC): Merged charts by category (PNG)
4. Analyzes market trends with LLM:
   - OEM trend (up=1, down=0)
   - Supplier trend (up=1, down=0)
   - Correlation (both up=1, one up/one down=0.5, both down=0)
5. Returns structured JSON with charts, data, scores, and market summary

Outputs saved under `outputs/`:
- stock_OEM_{ticker}_{timestamp}.png (individual OEM charts)
- stock_Battery_merged_{timestamp}.png (all battery suppliers)
- stock_HVAC_merged_{timestamp}.png (all HVAC suppliers)
- stock_analysis.json (structured data + scores)
"""
...
def run_stock_analysis(llm=None, out_dir: Optional[str] = None) -> Dict[str, Any]:
    ...
    Returns:
        {
            "oem_charts": {ticker: path, ...},
            "supplier_charts": {"Battery": path, "HVAC": path},
            "oem_data": [...],
            "supplier_data": [...],
            "trend_indicators": {...},
            "llm_evaluation": {...}
