...
- Runs 4 sub-agents in parallel (async)
- Validates each agent's output using LLM
- Retries failed agents (max 2 retries)
- Returns 1 if all validations pass, 0 with error info if failed

LangGraph flow:
    run_agents (parallel) -> validate_results -> [conditional: retry or finish]

Requirements:
    - All 4 agents must be importable
...
def run_supervisor(
    ...
    """
    Run SupervisorAgent to coordinate all sub-agents.
    
    Args:
        companies: List of OEM company names to analyze
        regions: List of region codes for ESG analysis (default: ["KR", "CN", "JP", "EU", "US"])
        out_dir: Output directory (default: ../outputs)
    
    Returns:
        {
            "final_status": 1 or 0,
            "validation_status": {agent: bool},
            "error_log": {agent: error_message},
            "companies": [...],
            "regions": [...],
            "tech_results": {...},
            "valuechain_results": {...},
